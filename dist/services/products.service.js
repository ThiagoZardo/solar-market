"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const products_model_1 = __importDefault(require("../database/models/products.model"));
const categories_model_1 = __importDefault(require("../database/models/categories.model"));
class ProductsService {
    constructor(productsModel = products_model_1.default) {
        this.productsModel = productsModel;
    }
    listAll() {
        return __awaiter(this, void 0, void 0, function* () {
            const products = yield this.productsModel.findAll({
                include: [
                    { model: categories_model_1.default, as: 'categoryInfo' },
                ],
            });
            return products;
        });
    }
    searchCategory(category) {
        return __awaiter(this, void 0, void 0, function* () {
            const searchCategory = yield this.productsModel.findAll({
                include: [{ model: categories_model_1.default, as: 'categoryInfo' }],
            });
            const search = searchCategory.filter((el) => {
                if (el.categoryInfo)
                    return el.categoryInfo.nameCategory === category;
                return 'Erro';
            });
            return search;
        });
    }
}
exports.default = ProductsService;
